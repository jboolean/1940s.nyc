...
- `sampling.ts`: loads eligible stories (state `published` or `rejected`, with text), splits them into the working and holdout pools, and draws random samples using lodash (`partition`, `sampleSize`, `compact`), which is already a dependency of this project.
